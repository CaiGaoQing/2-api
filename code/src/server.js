...
import { setupGrokRoutes } from './grok/grok-routes.js';
import { setupKraterRoutes } from './krater/krater-routes.js';
import { setupDirectRoutes } from './direct/direct-routes.js';
import { setupOutlookRoutes } from './outlook/outlook-routes.js';
...
app.get('/v1/models', async (req, res) => {
    ...
    // 追加 Grok 模型
    const { GROK_MODELS } = await import('./grok/grok-service.js');
    const grokModelList = GROK_MODELS.map(id => ({
        id,
        object: 'model',
        created: Math.floor(Date.now() / 1000),
        owned_by: 'xai',
    ...
    res.json({
        object: 'list',
        data: [...modelList, ...amiModelList, ...grokModelList]
    });
});
...
// 批量封禁检测 (SSE 流式返回进度) - 必须在 :id 路由之前注册
app.get('/api/credentials/batch-ban-check', async (req, res) => {
    res.setHeader('Content-Type', 'text/event-stream');
    res.setHeader('Cache-Control', 'no-cache');
    res.setHeader('Connection', 'keep-alive');

    try {
        const allCredentials = await store.getAll();
            ...
            while (queue.length > 0) {
                ...
                try {
                    ...
                    if (cred.refreshToken) {
                        const refreshResult = await KiroAPI.refreshToken(cred);
                        if (refreshResult.success) {
                            ...
                            // 刷新成功，尝试获取用量
                            const updatedCred = await store.getById(cred.id);
                            try {
                                const client = new KiroClient({
                                    accessToken: updatedCred.accessToken,
                                    refreshToken: updatedCred.refreshToken,
                                    profileArn: updatedCred.profileArn,
                                    region: updatedCred.region,
                                    authMethod: updatedCred.authMethod,
                                    clientId: updatedCred.clientId,
                                    clientSecret: updatedCred.clientSecret,
                                    expiresAt: updatedCred.expiresAt
                                });
                                ...
                                const code = usageErr.response?.status;
                                const msg = usageErr.response?.data?.message || usageErr.message;
                                if (code === 423 || code === 403 || msg.includes('TEMPORARILY_SUSPENDED') || msg.includes('Locked')) {
                                    status = 'banned';
                                    message = msg;
                                    bannedCount++;
                                    await store.moveToError(cred.id, `账号被封禁: ${msg}`);
                                } else {
                                    status = 'error';
                                    message = msg;
                                    errorCount++;
                                    await store.moveToError(cred.id, `获取用量失败: ${msg}`);
                                }
                            }
                        } else {
                            const errorMsg = refreshResult.error || '';
                            const code = refreshResult.statusCode;
                            if (code === 423 || code === 403 || errorMsg.includes('TEMPORARILY_SUSPENDED') || errorMsg.includes('Locked') || errorMsg.includes('banned') || errorMsg.includes('suspended')) {
                                status = 'banned';
                                message = errorMsg;
                                bannedCount++;
                                await store.moveToError(cred.id, `账号被封禁: ${errorMsg}`);
                            } else {
                                status = 'refresh_failed';
                                message = errorMsg;
                                errorCount++;
                                await store.moveToError(cred.id, `Token 刷新失败: ${errorMsg}`);
                            }
                        }
                    } else {
                        status = 'no_refresh_token';
                        message = '无 refreshToken';
                        errorCount++;
                        await store.moveToError(cred.id, '无 refreshToken');
                    }
                } catch (err) {
                    status = 'error';
                    message = err.message;
                    errorCount++;
                    await store.moveToError(cred.id, `检测异常: ${err.message}`);
                }
                ...
                sendEvent({
                    ...
                    checked,
                    total,
                    current: { id: cred.id, email: cred.name || cred.email, status, message },
                    bannedCount,
                    activeCount,
                    errorCount
                });
        ...
        sendEvent({ type: 'done', total, bannedCount, activeCount, errorCount });
        res.end();
    } catch (error) {
...
// 批量刷新 Token + 用量 (SSE 流式返回进度) - 必须在 :id 路由之前注册
app.get('/api/credentials/batch-refresh', async (req, res) => {
    res.setHeader('Content-Type', 'text/event-stream');
    res.setHeader('Cache-Control', 'no-cache');
    res.setHeader('Connection', 'keep-alive');
    ...
    try {
                    ...
                    if (!cred.refreshToken) {
                        ...
                        if (refreshResult.success) {
                            ...
                            try {
                                ...
                                usage = await client.getUsageLimits();
                                await store.updateUsage(cred.id, usage);
                                status = 'success';
                                message = 'Token 刷新成功，用量已更新';
                                successCount++;
                            } catch (usageErr) {
                                const code = usageErr.response?.status;
                                const msg = usageErr.response?.data?.message || usageErr.message;
                                if (code === 423 || code === 403 || msg.includes('TEMPORARILY_SUSPENDED') || msg.includes('Locked')) {
                                    status = 'banned';
                                    message = `账号被封禁: ${msg}`;
                                    await store.moveToError(cred.id, message);
                                    failCount++;
                                } else {
                                    status = 'usage_failed';
                                    message = `Token 已刷新，但获取用量失败: ${msg}`;
                                    successCount++; // Token 刷新成功了
                                }
                            }
                        } else {
                            const errorMsg = refreshResult.error || '';
                            const code = refreshResult.statusCode;
                            if (code === 423 || code === 403 || errorMsg.includes('TEMPORARILY_SUSPENDED') || errorMsg.includes('Locked') || errorMsg.includes('banned')) {
                                status = 'banned';
                                message = `账号被封禁: ${errorMsg}`;
                                await store.moveToError(cred.id, message);
                            } else {
                                status = 'refresh_failed';
                                message = errorMsg;
        ...
        sendEvent({ type: 'done', total, successCount, failCount });
        res.end();
    } catch (error) {
        res.write(`data: ${JSON.stringify({ type: 'error', message: error.message })}\n\n`);
        res.end();
...
// 单个账号封禁检测
app.post('/api/credentials/:id/ban-check', async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        let credential = await store.getById(id);
        ...
        if (credential.refreshToken) {
            ...
            if (refreshResult.success) {
                ...
                // Token 刷新失败，检查是否是封禁
                const errorMsg = refreshResult.error || '';
                const statusCode = refreshResult.statusCode;
                const isBanned = statusCode === 423 || statusCode === 403 ||
                    errorMsg.includes('TEMPORARILY_SUSPENDED') ||
                    errorMsg.includes('Locked') ||
                    errorMsg.includes('banned') ||
                    errorMsg.includes('suspended');

                if (isBanned) {
                    await store.moveToError(id, `账号被封禁: ${errorMsg}`);
                    return res.json({
                        success: true,
                        data: { status: 'banned', message: errorMsg, statusCode }
                    });
                }

                // Token 刷新失败也移到错误表
                await store.moveToError(id, `Token 刷新失败: ${errorMsg}`);
                return res.json({
                    success: true,
                    data: { status: 'refresh_failed', message: errorMsg, statusCode }
                });
            }
        }
        ...
        try {
            ...
            res.json({
                ...
                data: {
                    status: 'active',
                    usage,
                    subscriptionType: subscriptionInfo?.subscriptionTitle || 'Free',
                    message: '账号正常'
                }
            });
        } catch (usageError) {
            const status = usageError.response?.status;
            const errorMsg = usageError.response?.data?.message || usageError.message;
            const isBanned = status === 423 || status === 403 ||
                errorMsg.includes('TEMPORARILY_SUSPENDED') ||
                errorMsg.includes('Locked');
            ...
            if (isBanned) {
                ...
                return res.json({
                    success: true,
                    data: { status: 'banned', message: errorMsg, statusCode: status }
                });
            }

            await store.moveToError(id, `获取用量失败: ${errorMsg}`);
            res.json({
                success: true,
                data: { status: 'error', message: errorMsg, statusCode: status }
            });
        }
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
...
async function start() {
    ...
    // 设置 Grok 路由
    const grokCredentialStore = await GrokCredentialStore.create();
    setupGrokRoutes(app, authMiddleware, verifyApiKey, apiLogStore, grokCredentialStore);
    console.log(`[${getTimestamp()}] Grok 服务已启动`);

    // 设置 Krater 路由
    const kraterCredentialStore = await KraterCredentialStore.create();
    setupKraterRoutes(app, authMiddleware, verifyApiKey, apiLogStore, kraterCredentialStore);
    console.log(`[${getTimestamp()}] Krater 服务已启动`);

    // 设置 Direct 转发路由
    const directChannelStore = await DirectChannelStore.create();
    setupDirectRoutes(app, authMiddleware, verifyApiKey, apiLogStore, directChannelStore);
    console.log(`[${getTimestamp()}] Direct 转发服务已启动`);

    // 设置 Outlook 邮箱管理路由
    const outlookAccountStore = await OutlookAccountStore.create();
    setupOutlookRoutes(app, authMiddleware, outlookAccountStore);
    console.log(`[${getTimestamp()}] Outlook 邮箱管理已启动`);
    ...
    app.listen(PORT, () => {
        ...
        console.log('[API]   Grok 格式:    /grok/v1/chat/completions');
        console.log('[API]   Krater 格式:  /k/v1/chat/completions, /k/v1/messages');
